...
let n_coherence =
  ...
  let formula = (imply (neg (eqn (param (paramref "i")) (param (paramref "j")))) (imply (eqn (var (arr [("n", [paramref "i"])])) (const _C)) (eqn (var (arr [("n", [paramref "j"])])) (const _I)))) in
...
let () = run_with_cmdline (fun () ->
  ...
  let cinvs_with_varnames, relations = find protocol
    ~murphi:(In_channel.read_all "n_mutualExCe.m")
  in
