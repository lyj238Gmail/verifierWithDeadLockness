...
let n_RI =
  ...
  let formula = (andList [(andList [(eqn (var (global "replace")) (const _REQUIRE)); (eqn (param (paramref "i")) (var (global "curNode")))]); (eqn (var (record [arr [("node", [paramref "i"])]; arr [("cache", [paramref "j"])]; global "state"])) (const _INVALID))]) in
...
let () = run_with_cmdline (fun () ->
  ...
  let cinvs_with_varnames, relations = anotherFind protocol
    ~murphi:(In_channel.read_all "godsont-sca.m") ~symMethod:(true);
  in
