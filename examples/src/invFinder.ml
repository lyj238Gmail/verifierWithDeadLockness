...
let tabular_expans (Rule(_name, _, form, _), crule, _, assigns) ~cinv =
  ...
  let inv_inst = (concrete_prop_2_form cinv)  in (*simplify (concrete_prop_2_form cinv)*)
  (* pre_cond *)
  let obligations =
...
let new_tabular_expans (Rule(_name, _, form, _), crule, _, assigns) ~cinv =
  let ConcreteRule(_, pfs_rule) = crule in
  let ConcreteProp(_, pfs_prop) = cinv in
...
let simplify_prop property =
	...
	if List.length pds > 0 then
      ...
      let frms=List.map ps ~f:(fun p ->  (neg (apply_form f ~p))) in
			let frms=List.filter ~f:(is_satisfiable ) frms in
			begin 	match frms with
					|x::xs -> [x]
			end 
  else 
      [ (neg f)]
...
let simplify_prop' property =
    ...
    if List.length pds > 0 then
      let ps = cart_product_with_paramfix pds (!type_defs) in
      List.map ps ~f:(fun p ->   (neg (apply_form f ~p)))
    else begin
      [  (neg f)]
    end
  in
...
let find ?(insym_types=[]) ?(smv_escape=(fun inv_str -> inv_str))
 ...
 (*  let ()=print_endline (ToStr.Smt2.context_of ~insym_types ~types ~vardefs) in*)
