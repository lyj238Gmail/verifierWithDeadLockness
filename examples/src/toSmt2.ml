...
let gen_var_str trees =
  ...
  let get_arr_of pds = String.concat ~sep:" " (
    List.map pds ~f:(fun (Paramdef(_, t)) -> sprintf " %s " t)
  ) in
  let rec wrapper forRecord tree =
    match tree with
    | Node(name, pds, [], tname) -> 
    	if (forRecord) 
    	then begin
    		if (List.length pds=0) 
    		then sprintf "(%s %s)" name tname  
    		else sprintf "(%s (Array %s %s))" name (get_arr_of pds) tname  (*%s : %s %s;*)
    		end
    	else begin
    		if (List.length pds=0) 
    		then sprintf "(declare-const %s %s)" name tname  
    		else sprintf "(declare-const %s (Array %s %s))" name   (get_arr_of pds) tname  (*%s : %s %s;*)
    		end 
        	...
        	then
        		begin
        		 if List.length pds=0 then
        		 	sprintf "( %s (  %s))" name     record_name 
        		 else	sprintf "( %s (Array %s %s))" name   (get_arr_of pds) record_name        (*( %s (Array %s %s)*)      (*%s : %s %s;*)
        		end
        	else 
        		begin
        			if List.length pds=0 then
        			sprintf "(declare-const %s  ( %s))" name    record_name    
        			else	sprintf "(declare-const %s  (Array %s %s))" name   (get_arr_of pds) record_name    
        		end
        | None ->
          let sub_items = List.map nodes ~f:(wrapper true) in
          let record_name = sprintf "Record_%d" (Hashtbl.length record_table) in
          let record_body = String.concat ~sep:"\n" sub_items in
          let record_str = sprintf "(declare-datatypes () ((%s (mk-%s %s))))" record_name record_name record_body in   (*%s : record\n%s\nend;*)
  ...
  in
  let vars = List.map trees ~f:(wrapper false )in
  (!records), vars
