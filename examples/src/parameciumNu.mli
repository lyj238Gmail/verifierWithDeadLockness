...
type typedef=
	|BasicType of string
	|Enum of string * const list
	|ArrayType of string*typedef  *typedef  (*typeName --index type- element type*)
	|Recordef of string*(string*typedef)list  (*typeName --(flieldName,fieldType) list*)
...
val getTypeName:typedef ->string 
...
*)
type vardef =
  | Vardef1 of string * string
  | Vardef2 of string * typedef
with sexp

val vardef1 : string ->string -> vardef
val vardef2 : string ->typedef -> vardef
...
val ident: string ->designator 
val field :designator -> string ->designator 
val arr :designator -> paramref -> designator 

val paramref:  string -> paramref
val paramfix:  string -> string -> exp-> paramref
...
val name2type : tname:string -> types:typedef list -> exp list
...
val apply_designator : designator -> p:paramref list -> designator
...
module VarNames : sig
  ...
  (** Names of var 
  val of_var : designator -> String.Set.t*)
...
module VarNamesWithParam : sig
  ...
  val of_exp : of_var:(designator-> String.Set.t) -> exp -> String.Set.t
  val of_form : of_var:(designator -> String.Set.t) -> formula -> String.Set.t
  val of_statement : of_var:(designator-> String.Set.t) -> statement -> String.Set.t
  val of_rule : of_var:(designator -> String.Set.t) -> rule -> String.Set.t
