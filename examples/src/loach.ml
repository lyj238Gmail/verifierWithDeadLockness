...
module Trans = struct
  ...
  *)
  ...
  and invTrans_formula   form =
	...
	let   invTrans_prop   p =
     let Paramecium.Prop(n,pfs,f) =p in
		Prop(n,pfs,invTrans_formula f)
end
...
let rec  onlyOnStatement index statement= 
	match statement with
	|Assign(var,value) ->  onlyOnVar index var 
	|Parallel(statementList) -> 
		List.fold  (List.map ~f:( onlyOnStatement index) statementList) 
		 ~f:(fun x y -> x || y) ~init:false
	|IfStatement(bcond,statement) ->
		 onlyOnStatement index statement
	|IfelseStatement(b,s1,s2) ->
	 ( onlyOnStatement index s1) || ( onlyOnStatement index s2)
...
let drawConclusion paramRef types invaraint=	
...
let drawConclusions paramRef types invs =
	 (List.map ~f:(drawConclusion paramRef types) invs) 
...
let rec cmpStrengthRule invs  ~types paramRef r=
	let Paramfix(pname,indextype,Intc(index))=paramRef in
	
		let invs=List.filter ~f:(relate ~rule:r ~types: types) invs in	
		match	invs with
		|[]->   r
		|_ ->
			let conclusions=List.concat (drawConclusions paramRef types invs)  in
			if conclusions=[] then r
			else
				...
				let invs'=List.filter ~f:(fun x->not (relate ~rule:r ~types: types x)) invs in
...
let cmpStrengthPrPInv pprops ~types paramRef pr=		
	let Paramfix(pname,indextype,Intc(index))=paramRef in
	let mkActualParasForInv pprop=
		let Prop( name, params, pinv)=pprop in
		let paraNumPairs=List.zip parasOfPInv (upto  (List.length parasOfPInv)) in
			List.map
			 ~f:(fun (p,num)->let Paramdef(pname,indextype)=p in Paramfix(pname,indextype,num+index))
			 paraNumPairs in 
	let pinvs=List.map (fun prop ->let Prop( name, params, pinv)=prop	 in pinv) pprops in
	let invs=List.concat (List.map (apply_form ~p:mkActualParasForInv)	pinvs	) in
