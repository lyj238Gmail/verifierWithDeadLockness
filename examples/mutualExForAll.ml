...
let n_coherence =
  ...
  let params = [paramdef "i" "client"] in
  let formula = (imply (eqn (var (global "x")) (const (boolc true))) (forallFormula [paramdef "j" "client"] (neg (eqn (var (arr [("n", [paramref "j"])])) (const _C))))) in
  prop name params formula

let n_coherence1 =
  let name = "n_coherence1" in
  let params = [paramdef "i" "client"] in
  let formula = (imply (eqn (var (global "x")) (const (boolc false))) (existFormula [paramdef "j" "client"] (orList [(eqn (var (arr [("n", [paramref "j"])])) (const _C)); (eqn (var (arr [("n", [paramref "j"])])) (const _E))]))) in
  prop name params formula

let properties = [n_coherence;n_coherence1]


let protocol = {
  name = "n_mutualExForAll";
  types;
  vardefs;
  init;
  rules;
...
let () = run_with_cmdline (fun () ->
  ...
  let cinvs_with_varnames, relations = anotherFind protocol
    ~murphi:(In_channel.read_all "n_mutualExForAll.m")
  in
