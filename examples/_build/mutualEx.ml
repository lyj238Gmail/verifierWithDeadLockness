...
let () = run_with_cmdline (fun () ->
  ...
  let cinvs_with_varnames, relations = anotherFind protocol ~symMethod:(false) ~symIndex:false
    (*~smv:(In_channel.read_all "mutualExOn.smv")*)
     ~murphi:(In_channel.read_all "n_mutualEx.m" )
  in 
  (*let cinvs_with_varnames, relations = find protocol ~symMethod:(false) ~symIndex:false*)
   (* ~smv:(In_channel.read_all "mutualExOnI.smv")*)
  (*  ~murphi:(In_channel.read_all "n_mutualEx.m")*)
  (*in*)
  (*let str=ToDafny.protocol_act protocol   in
  let ()=print_endline str in*)
  Coq.protocol_act protocol cinvs_with_varnames relations ()
)
