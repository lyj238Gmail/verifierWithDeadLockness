...
let is_tautology ?(quiet=true) form =
 (* let ()=printf "\n---%s---\n" (ToStr.Debug.form_act form) in *)
  not (Smt.is_satisfiable ~quiet (ToStr.Another1Smt2.form_of (neg form)))


(** Judge if a formula is satisfiable *)
let is_satisfiable ?(quiet=true) form =
	(*let ()=print_endline (ToStr.Debug.form_act form) in*)
  Smt.is_satisfiable ~quiet (ToStr.Another1Smt2.form_of form)
...
let form2AllSymForm ~f ~types=
	 ...
	 let tmp1=List.map ~f:(fun x->print_endline (ToStr.Debug.paramdef_act x)) pds in
	 let tmp2=List.map ~f:(fun x->print_endline (ToStr.Debug.paramref_act x)) prs in
	 let tmp3=print_endline (ToStr.Debug.form_act pf) in
	 match pds with
	 |[] -> [f]   
	 | _ ->
